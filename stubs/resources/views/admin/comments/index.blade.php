            ...
            <div class="w-full max-w-full flex-none px-3">
                <div
                    ...
                    @if(Session::has('success'))
                        <div
                            class="mx-auto flex h-10 w-full items-center justify-center rounded-tl-lg rounded-tr-lg bg-lime-500 px-6 text-white"
                            id="flash-container">
                            <p class="text-sm font-normal capitalize">{{Session::get('success')}}</p>
                        </div>
                    @endif
                    ...
                    <div class="flex-auto px-0 pt-0 pb-2">
                        <div class="overflow-x-auto p-0">
                            <table
                                ...
                                @foreach($comments as $comment)
                                    <tr class="hover:bg-gray-50">
                                        <td class="whitespace-nowrap max-w-sm border-t bg-transparent p-2 align-middle shadow-transparent comment-body hover:cursor-pointer">
                                            <div class="flex px-2 py-1">
                                                <div class="flex flex-col justify-center">
                                                    <p class="text-xssm font-normal">{{$comment->body}}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="whitespace-nowrap border-t bg-transparent p-2 align-middle shadow-transparent">
                                            @if($comment->user)
                                                <div class="flex px-2 py-1">
                                                    <div>
                                                        <img
                                                            src="{{($comment->user->media->first()->url ?? '')}}"
                                                    ...
                                                    <div class="flex flex-col justify-center">
                                                        <p class="text-xssm">{{$comment->user->name, 20}}</p>
                                                    </div>
                                                </div>
                                            @else
                                                <p class="text-xssm"> - </p>
                                        ...
                                        <td class="whitespace-nowrap border-t bg-transparent p-2 text-center align-middle text-sm leading-normal shadow-transparent">
                                            <a href="{{route('post.show', [$comment->post->slug, '#comment-'.$comment->id])}}"
                                               class="text-xs text-blue-400 hover:text-blue-600 capitalize font-normal">{{\Illuminate\Support\Str::limit($comment->post->title, '30', '...')}}</a>
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap border-t bg-transparent p-2 text-center align-middle text-sm leading-normal shadow-transparent">
                                            <div class="flex items-center justify-center">
                                                <form action="{{route('comment.approve', $comment)}}" method="POST"
                                                      comment-approval>
                                                    <div class="max-w-md">
                                                        <x-admin.radio-toggler label="" value="{{$comment->approved}}"/>
                                                    @csrf
                                                </form>
                                            </div>
                                        </td>
                                        ...
                                        <td class="whitespace-nowrap border-t bg-transparent p-2 align-middle shadow-transparent">
                                            <div class="flex w-1/2 items-center justify-between">
                                                @can(\App\Constants\Permissions::DELETE_COMMENTS)
                            ...
                            <x-admin.delete-action-confirmation-modal id="comments"></x-admin.delete-action-confirmation-modal>
