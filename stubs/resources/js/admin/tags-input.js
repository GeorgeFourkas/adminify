            ...
            .then(response => {
                    ...
                    response.data.tags.forEach((item) => {
                        suggestContainer.insertAdjacentHTML('beforeend', `
                                  <li class="relative cursor-pointer select-none py-2 pr-9 pl-3 text-gray-900 hover:from-adminify-main-color hover:to-adminify-secondary-color hover:bg-gradient-to-tl hover:text-white"
...
function setSelectedTag(databaseTagName, databaseId) {
    selectedTags.push({
        ...
        get pill() {
            return `
              <div class="m-1 cursor-pointer rounded-md bg-green-100 p-1 text-center group hover:from-adminify-main-color hover:to-adminify-secondary-color hover:bg-gradient-to-tl" tag-pill>
